...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === '야바위') {
    ...
    const rowButtons = new ActionRowBuilder().addComponents(
      cards.map((card, i) => new ButtonBuilder().setCustomId(`yabawi_${i}_${cards.join('')}_${bet}`).setLabel(`카드 ${i + 1}`).setStyle(ButtonStyle.Primary))
    );

    return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR_INFO).setTitle("🎲 야바위 게임").setDescription("3장의 카드 중 하나를 선택하세요!")], components: [rowButtons] });
  }
  ...
  if (commandName === '랭킹') {
    ...
    const rows = res.rows;
    if (!rows || rows.length === 0) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR_ERROR).setTitle("📉 데이터 없음")] });
    ...
    if (type === 'server') {
      ...
      return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR_INFO).setTitle(`⭐ ${guild.name} 서버 랭킹`).setDescription(rankMsg)] });
    }
    ...
    if (type === 'global') {
      ...
      return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR_INFO).setTitle("🏆 전체 서버 랭킹").setDescription(rankMsg)] });
    }
  }
  ...
  if (commandName === '청소') {
    ...
    const target = options.getUser('유저');
    if (amount < 1 || amount > 100) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR_ERROR).setTitle("❌ 범위 오류").setDescription("1~100개까지만 삭제할 수 있습니다!")] });
