  ...
  if (interaction.isChatInputCommand()) {
    ...
    // ======================
      ...
      db.get("SELECT balance FROM users WHERE id = ?", [user.id], (err, row) => {
        ...
        if (isNaN(bet) || bet <= 0 || row.balance < bet) {
          const embed = new EmbedBuilder()
            .setColor(COLOR_ERROR)
            .setTitle("❌ 실패")
    ...
    // ======================
      ...
      const betInput = options.getString('금액'); // 올인/숫자 입력
      ...
      db.get("SELECT balance FROM users WHERE id = ?", [user.id], (err, row) => {
        ...
        if (isNaN(bet) || bet < 1000 || row.balance < bet) {
          const embed = new EmbedBuilder()
            .setColor(COLOR_ERROR)
            .setTitle("❌ 실패")
            .setDescription("잔액 부족, 계정 없음, 최소 베팅 1000 이상이어야 합니다.");
          return interaction.editReply({ embeds: [embed] });
        }
      ...
      if (type === 'server') {
        ...
        db.all("SELECT id, balance FROM users WHERE balance > 0 ORDER BY balance DESC LIMIT 10", (err, rows) => {
          ...
          let rankMsg = rows.map((row, i) => {
            ...
            const name = userObj?.username || row.id;
            const medal = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i + 1}`;
            return `${medal} ${name} — ${fmt(row.balance)} 코인 💰`;
          }).join("\n");
