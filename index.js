...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === '돈내놔') {
    ...
    if (!row) {
      ...
      return interaction.editReply({
        embeds: [new EmbedBuilder().setColor(COLOR_ADMIN).setAuthor({ name: nick, iconURL: avatar(guild, user.id) }).setTitle("🎉 첫 보상 지급 완료! 🎉").setDescription("💰 20,000 코인 지급!")]
      });
    }

    if (row.lastdaily === today) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setColor(COLOR_ERROR).setTitle("⏳ 이미 받음").setDescription("오늘은 이미 받았습니다. 내일 00:00 이후 다시 시도하세요!")]
      });
    }
    ...
    return interaction.editReply({
      embeds: [new EmbedBuilder().setColor(COLOR_SUCCESS).setAuthor({ name: nick, iconURL: avatar(guild, user.id) }).setTitle("💸 돈 지급 완료!").setDescription(`잔액: ${fmt(newBalance)} 코인`)]
    });
  }
    ...
    return interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setColor(COLOR_INFO)
          .setTitle("💰 현재 잔액 💰")
          .setFooter({ text: `${nick} ｜ ${fmt(row.balance)} 코인`, iconURL: avatar(guild, user.id) })
      ]
    });
  }
  ...
  if (commandName === '송금') {
    ...
    if (!target || amount <= 0 || user.id === target.id) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setColor(COLOR_ERROR).setTitle("❌ 송금 불가").setDescription("자기 자신에게는 송금 불가, 금액은 1 이상이어야 합니다.")]
      });
    }
    ...
    return interaction.editReply({
      embeds: [
        new EmbedBuilder()
          ...
          .setFooter({
            text: `${guild?.members.cache.get(target.id)?.displayName || target.username} ｜ 💰 ${fmt(amount)} 코인`,
  ...
  if (commandName === '동전던지기') {
    ...
    if (!row) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR_ERROR).setTitle("❌ 실패").setDescription("계정 없음. `/돈내놔` 먼저!")] });

    let bet = (betInput === "올인") ? row.balance : parseInt(betInput, 10);
    if (!Number.isFinite(bet) || bet <= 0 || row.balance < bet) {
      return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR_ERROR).setTitle("❌ 실패").setDescription("잔액 부족 혹은 금액 오류.")] });
    }
    ...
    return interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setColor(result === side ? COLOR_SUCCESS : COLOR_ERROR)
          .setTitle(result === side ? "📌 승리 🎉" : "📌 패배 ❌")
  ...
  if (commandName === '대박복권') {
    ...
    if (!Number.isFinite(bet) || bet < 1000 || row.balance < bet) {
      return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR_ERROR).setTitle("❌ 실패").setDescription("잔액 부족, 최소 베팅 1000 이상")] });
    }
    ...
    return interaction.editReply({
      embeds: [
        new EmbedBuilder()
          ...
          .setTitle(payout > 0 ? `📌 당첨결과 ${result} ${PAYOUTS[result]}배!` : `📌 당첨결과 ${result} 꽝!`)
          .setDescription(payout > 0 ? `획득 +${fmt(payout - bet)} 코인` : `손실 -${fmt(bet)} 코인`)
  ...
  if (interaction.isButton() && interaction.customId.startsWith('yabawi')) {
    ...
    if (!row || row.balance < wager) {
      return await interaction.reply({ embeds: [new EmbedBuilder().setColor(COLOR_ERROR).setTitle("❌ 오류").setDescription("잔액 부족 또는 계정 없음")], ephemeral: true });
    }
