...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    // /동전던지기 (빠르니까 reply)
      ...
      db.get("SELECT balance FROM users WHERE id = ? AND guildId = ?", [user.id, guild.id], (err, row) => {
        if (!row) return interaction.reply("❌ 먼저 `/돈내놔`로 계정을 생성하세요!");
        if (bet <= 0) return interaction.reply("❌ 베팅 금액은 1 이상이어야 합니다!");
        if (row.balance < bet) return interaction.reply("❌ 코인이 부족합니다!");

        const result = Math.random() < 0.5 ? '앞면' : '뒷면';
        let newBalance = row.balance;
        if (result === side) {
          newBalance += bet;
          interaction.reply(`🎉 ${result}! 승리! +${fmt(bet)}\n현재 잔액: ${fmt(newBalance)}`);
        } else {
          newBalance -= bet;
          interaction.reply(`😢 ${result}! 패배... -${fmt(bet)}\n현재 잔액: ${fmt(newBalance)}`);
        }
    ...
    // /대박복권
      ...
      await interaction.reply("⌛ 복권 추첨 중...");
    ...
    // /야바위
      ...
      await interaction.reply("⌛ 야바위 진행 중...");
      const bet = options.getInteger('금액');
    ...
    // /송금
      ...
      await interaction.reply("⌛ 송금 처리 중...");
    ...
    // /랭킹
      ...
      await interaction.reply("⌛ 랭킹 계산 중...");
